...
- Session summary: Recovered Obsidian LiveSync after the recent CouchDB migration to CT128 (root cause was the `livesync` non-admin user lacking access to the recreated `obsidian_main` DB; restored via DELETE+PUT+`_security` member grant on CT128, then Mac "Overwrite remote" pushed all 6,484 docs back). Then deployed CT175 `obsidian` as a new repo-managed LXC running the `linuxserver/obsidian` Webtop image, joining the existing CT128 LiveSync mesh as another peer alongside Mac/phone, with the Local REST API plugin destined for `obsidian-api.laxdog.uk` so MCP servers can reach the vault. Repo changes shipped in commit `98e7793`: new `obsidian` Ansible role, Terraform LXC entry, AdGuard rewrites, NPM proxy hosts + cert SAN expansion, vaulted webtop password, `docs/obsidian.md` runbook for the unavoidable GUI bootstrap (paste setup URI on Mac, install Self-hosted LiveSync + Local REST API plugins, extract API key, vault it). Multi-host `--limit` apply hit two pre-existing role bugs (NPM `Update`-task halt blocked the `Create-missing` task from materialising the new internal proxy hosts, and the AdGuard role's template render appeared to be silently skipped on the first run) — both worked around by hand and recorded in `docs/backlog.md` with proposed fixes. Pending operator steps: DHCP reservation for `bc:24:11:cb:80:79` on the ASUS router, and the GUI bootstrap in `docs/obsidian.md`. Still untouched: `ansible/inventory.yml` is auto-regenerated by `scripts/run.py` and was already dirty before the session — that diff is unrelated to this work.
