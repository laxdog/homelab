...
## 2026-04-29
- Tdarr docs and media-stack agent notes were reconciled against live VM120 state. Runtime verification on `media-stack` (`10.20.30.120`) confirmed `tdarr` + `tdarr-node` are both up in the repo-managed `arr` compose project, Tdarr API health is good on `2.67.01`, the live node `MediaStackTdarrCpuNode` is currently paused with runtime node id `gps47_OkU`, and the Tdarr DB still contains the guarded validation libraries plus the staged Mr. Robot sample output from the 2026-04-07 rollout. Docs now explicitly distinguish the live runtime node id from the older `QyKZlyoUY` value preserved in historical staged/job records, and the stale media-stack note claiming compose files still only lived on VM120 was corrected.

## 2026-04-27
- CT175 `obsidian` deployed: `linuxserver/obsidian` Webtop (full Obsidian.app via KasmVNC) on a new repo-managed LXC (id 175, 10.20.30.175, 2 GB RAM, 16 GB ssd-mirror). Joins the existing CT128 `obsidian_main` CouchDB DB as another LiveSync peer alongside the Mac/phone, and exposes the Local REST API plugin at `obsidian-api.laxdog.uk` for MCP integrations. Internal-only via `obsidian.laxdog.uk` (Webtop, basic auth) and `obsidian-api.laxdog.uk` (REST API, plugin API key). Repo-managed: new `obsidian` Ansible role (mirrors `couchdb`/`freshrss` shape), Terraform LXC entry, AdGuard rewrites, NPM proxy hosts + cert SAN expansion, Heimdall/Organizr exclusion for the API hostname, vaulted `obsidian_webtop_password`. Bootstrap (vault open, plugin install, setup-URI paste, REST API key extraction) is one-time GUI work documented in `docs/obsidian.md` because the API key is generated on first plugin start and can't be pre-seeded. Pending: DHCP reservation for `bc:24:11:cb:80:79` on the ASUS router.
- Two pre-existing role bugs surfaced during the multi-host apply for the deploy above. Both have been worked around manually for this rollout but not fixed in the role itself — added to backlog. (1) **NPM proxy host Update task hangs on existing external hosts**: while issuing the new 33-domain LE cert (laxdog-internal-le, id 24, now includes `obsidian.laxdog.uk` + `obsidian-api.laxdog.uk`), NPM's backend was OOM-killed (`max_old_space_size=250` MB) under the cert + nginx-reload load. After NPM restarted, subsequent `PUT /api/nginx/proxy-hosts/N` calls for external (`*.lax.dog`) hosts started returning 502, halting the role's loop and blocking the "Create AdGuard proxy host when missing" task — meaning the new Obsidian internal proxy hosts never got created. Worked around by creating proxy hosts 65 + 66 directly via NPM's `POST /api/nginx/proxy-hosts` (cert 24, access list 2 = Internal Access). (2) **AdGuard role's template render task silently skipped on the multi-host apply**: first-run recap reported `adguard ok=9 changed=0` and `/opt/AdGuardHome/AdGuardHome.yaml` was untouched (mtime still 2026-04-21), so the new `obsidian.laxdog.uk` and `obsidian-api.laxdog.uk` rewrites weren't rendered. Re-running with `--limit adguard` produced `ok=42 changed=3` and the rewrites appeared as expected. Repro mechanism unclear — possibly related to the NPM play's failure earlier in the same playbook run.

## 2026-04-23
- Home Assistant gained a dedicated repo-managed Lights dashboard at `/house-lights/overview`. It is generated from `home_assistant.lights_dashboard` and synced by `python3 scripts/home_assistant.py sync-lights-dashboard`, using capability-aware Mushroom light cards so only supported controls are shown per light. Current scope is practical day-to-day control for the user-facing house lights; the dedicated status bulb remains intentionally excluded from that dashboard.
- VM171 Mullvad egress ~30 h silent outage resolved, and `mullvad-exit` role hardened to prevent recurrence. Root cause: `unattended-upgrades` on 2026-04-22 06:32:21–38 upgraded `libcap2`/`libpam-cap`/`ntfs-3g`, which triggered `systemctl daemon-reexec` → `systemd-networkd` restart → kernel ip-rules flushed. Tailscale's daemon self-restores its own pri 5210–5270 rules; nothing was restoring `wg-quick`'s pri 5208/5209 or the role's PostUp pri 5200 (tailnet-CGNAT coexistence) rule. With those gone, VM171's own traffic fell back to home NAT (`212.56.120.65`) and exit-node-forwarded traffic from `rr-worker-staging-home` black-holed at the kill-switch's tail DROP. Detected 2026-04-23 14:47 while auditing tailscale runtime prefs; fixed in-place with `systemctl restart wg-quick@wg0`. Durability fix: `mullvad-exit` role now installs `/etc/systemd/system/wg-quick@<iface>.service.d/restart-on-networkd.conf` with `PartOf=systemd-networkd.service`, so wg-quick follows networkd's lifecycle and re-lays all ip-rules on every networkd bounce. Verified by deliberate `systemctl restart systemd-networkd` — `wg-quick@wg0` restarted, all rules and routes came back, egress returned to `141.98.252.208` (`gb-lon-wg-001`), kill-switch chain intact.
