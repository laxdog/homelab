...
5. **DNS and access** — choose the right domain:
   - **Internal only** (`laxdog.uk`): add AdGuard rewrite (`subdomain.laxdog.uk → 10.20.30.154`) + NPM proxy host. Add the new subdomain to cert 17's SAN list via `certbot --expand` in the NPM container (DNS-01 via Cloudflare API). No Cloudflare A records needed.
...
8. Add DHCP reservation to the router: `ssh admin@10.20.30.1` → add `<MAC>IP>>` to `nvram dhcp_staticlist` → `nvram commit` → `service restart_dnsmasq`
9. Run `terraform plan` -> `terraform apply`
10. Run Ansible playbook if a role is needed
